...
template<typename TY, const unsigned CH, typename CM>
class   WAVELIB_API StepflowFX;
template<typename TY, const unsigned CH, typename PZ>
...
WAVELIB_API WavFileHeader CreateWaveHeader( i32 samplerate, i16 bitdepth,
                                            i16 channelcount, WavFileHeader* in_waveFileHeader,
                                            i32 sizeOfWaveData );
// Construct a WaveHeader (file header) structure
WAVELIB_API WavFileHeader CreateWaveHeader( i32 samplerate, i16 bitdepth,
                                            i16 channelcount, i32 sizeOfWaveData );
...
WAVELIB_API Format        CreateWaveFormat( uint typecode, int samplerate );
...
#ifdef PAM_WAVEFILE_HEADERS
WAVELIB_API PamFileHeader CreatePamFileHdr( const Format& format, int framecount, ChannelMode planar );
#endif
